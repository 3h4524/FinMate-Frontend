...
const showLoading = () => {
  showElement(document.getElementById('loadingDiv'));
  DOM.transactionTableBody.innerHTML = '';
  hideElement(DOM.paginationDiv);
};
const hideLoading = () => {
  hideElement(document.getElementById('loadingDiv'));
};
...
const renderPagination = (total) => {
  ...
  DOM.nextBtn.disabled = state.currentPage === state.totalPages - 1;
...
const openTransactionModal = async (transaction = null) => {
  state.editingTransactionId = transaction ? transaction.transactionId : null;
...
const initEventListeners = () => {
  ...
  DOM.transactionTableBody.addEventListener('click', async (e) => {
    ...
    if (type === 'transaction') {
      ...
      const data = await response.json();
      if (data.code === 1000) {
        openTransactionModal(data.result);
      }
    } else if (type === 'delete') {
